...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # Here we are using the function's on our proxy_socket object of class socket Bind(tuple()) and listen(int)
        # - Bind(tuple) recives a tuple containing (ip address , port) this assigns an ip address and a port number to our socket object
        # in our case the paremeter proxy_address is a tuple that satisfys the function args and is passed through
        # - listen(int) receives an integer stating the maximum amount of clients we want our proxy_server to handle simuoltanuosly
        # When our proxy_server is handling its max amount of clients other clients trying to connect to the server will be put on hold
        # This is done by threading and creates a new thread for each client - proxy_server relationship.
        #
        # * Fill in start (1)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # Here we are saving to the variable data = the recived message of size 4096 (better if size is a power of 2) from the client_socket object
            ...
            try:
                ...
                # Here we are sending back the packed response back to the client
                # * Fill in start (4)
