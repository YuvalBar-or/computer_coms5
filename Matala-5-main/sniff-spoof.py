...
from scapy.all import sniff,send, TCP, UDP, ICMP, IP
import datetime
...
class PacketSniffer:
    def __init__(self, proto):
        self.file_name = "attacker docker.txt"
        self.protocol = proto
    ...
    def sniff_packets(self):
        sniff(filter=self.protocol, prn=self.process_packet, store=False, iface="vethf2ffc22")
    ...
    def parse_tcp_packet(self, packet):
        ...
        sniffer.save_packet_data(parsed_data)
    ...
    def parse_icmp_packet(self, packet):
        ...
        if packet[ICMP].type == 8:
                print("in")
                newpacket = IP(src=packet[IP].dst, dst=packet[IP].src) / ICMP()
                icmp = ICMP(type = 0, id=packet[ICMP].id, seq=packet[ICMP].seq)
                spoofed = newpacket / icmp
                send(spoofed)
        sniffer.save_packet_data(parsed_data)
